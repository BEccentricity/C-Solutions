...
template <typename D, typename T, bool C = is_empty_v<D>> struct Has;

template <typename D, typename T>
struct Has<D, T, false>
{
private:
    static constexpr bool current_match = std::is_same_v<front<D>, T>;
    static constexpr bool rest_match = Has<pop_front<D>, T>::value;
    
public:
    static constexpr bool value = current_match || rest_match;
};
...
template <typename D, typename T>
inline constexpr bool has_v = Has<D, T>::value;
...
int main()
{
    ...
    static_assert(!has_v<Deque<>, int>);
    static_assert(has_v<Deque<int>, int>);
    static_assert(!has_v<Deque<int>, double>);
    
    static_assert(has_v<Deque<int, double>, int>);
    static_assert(has_v<Deque<int, double>, double>);
    static_assert(!has_v<Deque<int, double>, char>);
    
    static_assert(has_v<Deque<int, double, char>, int>);
    static_assert(has_v<Deque<int, double, char>, double>);
    static_assert(has_v<Deque<int, double, char>, char>);
    static_assert(!has_v<Deque<int, double, char>, float>);
    
    static_assert(has_v<Deque<int, int, int>, int>);
    static_assert(!has_v<Deque<int, int, int>, double>);
