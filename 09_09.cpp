...
public:
    ...
    Allocator(std::size_t size, Algorithm algorithm = Algorithm::First) : m_size(size), m_algorithm(algorithm)
    {
...
private:
    ...
    {
        Node * current = m_head, * previous = nullptr;
        Node * best_current = nullptr, * best_previous = nullptr;
        std::size_t best_size = ~0uz;
        ...
        return std::make_pair(best_current, best_previous);
    }

    auto find(std::size_t size) const -> std::pair < Node *, Node * >
    {
        if (m_algorithm == Algorithm::First)
        {
...
void test_v2_first(benchmark::State & state)
{
    ...
    for (auto element : state)
    {
        Allocator allocator(16 * gb, Allocator::Algorithm::First);
...
void test_v2_best(benchmark::State & state)
{
    auto kb = 1uz << 10, mb = 1uz << 20, gb = 1uz << 30;
    ...
    for (auto element : state)
    {
        Allocator allocator(16 * gb, Allocator::Algorithm::Best);
...
BENCHMARK(test_v1);
BENCHMARK(test_v2_first);
BENCHMARK(test_v2_best);

int main()
{
    Allocator allocator(1 << 10, Allocator::Algorithm::First);
